...
class VotingCardController extends Controller
{
    ...
    function __construct()
    {
		...
		$this->module_url_path              = $this->admin_url_path."/voting_card";
		$this->module_title                 = "Voter";
		$this->module_view_folder           = "admin.voting_card";
...
// dd($this->arr_view_data)
    ...
    public function load_data(Request $request,$type=null)
    {
    	...
    	$obj_user     = DB::table($user_details)
    				    ...
    				    ->where($user_details.'.voter_id','=',null)
			...
			foreach ($build_result->data as $key => $data) 
			{
				if(isset($build_result->data) && sizeof($build_result->data)>0)
				{
    				...
    				$id      = isset($data->id)? base64_encode($data->id):'';
						...
						{
							$built_voting_card_button = "
							<a class='btn btn-default btn-rounded btn-sm show-tooltip edit_button' data-id='".$id."' title='Add Voting Card' id='add_voting_card' data-original-title='Add Voting Card'><i class='fa fa-plus' ></i> Add Voting Card</a>
							";
						}
						else
						{
						...
						if(get_admin_access('voting_card','edit'))
						{
							$built_edit_button = "
						...
						if(get_admin_access('voting_card','view'))
						{
							$built_view_button = " 
                    ...
                    $action_button_html .= '<li>'.$built_voting_card_button.'</li>';
					...
					$built_action_button= $built_voting_card_button.$built_view_button.$built_edit_button.$built_delete_button;
	...
	public function update_voting_card(Request $request)
	{
		// dd($request->all());
		...
		$status = $this->BaseModel->where('id', base64_decode($id))->update($arr_data);
...
/*
	public function add_aadhar(Request $request)
	{ 
		$arr_rules      = $arr_data = array();
		$status         = false;
		$arr_rules['aadhar_id']      	   	 = "unique:users,aadhar_id";
		...
		$status = $this->BaseModel->where('id', base64_decode($user_id))->update($arr_data);
			// dd($status)	 ;
		if($status)
		{
			Session::flash('success', str_singular($this->module_title).' Id Added successfully.');
			return redirect($this->module_url_path);
		}
		Session::flash('error', 'Error while creating '.str_singular($this->module_title).'.');
		return redirect($this->module_url_path);
