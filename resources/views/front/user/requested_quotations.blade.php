...
<section class="gray-bg-main-section">
                ...
                <div class="col-sm-8 col-md-8 col-lg-10 ">
                    <div class="filter-search-main">                            
                        <div class="form-group">
                            ...
                            <input type="search" name="search" type="text" placeholder="Search Quotations by Id or Aircraft model" value="{{isset($_GET['search'])?$_GET['search']:''}}"  id="search">                            
                        </div>
            ...
            <div class="main-quote request-quote-main booking-pending">
                <div class="img-quote">
                    ...
                    @elseif($value['status'] == 'REPLIED')
                        <div class="listing-persent">
                            <a href=" {{url('/')}}/user/accept_quotation/{{base64_encode($value['rfq_id'])}}" title="Accept" class="edit-icon"><i class="fa fa-check"></i></a>
                            <a href="{{url('/')}}/user/reject_quotation/{{base64_encode($value['rfq_id'])}}" title="Reject" class="delete-icon"><i class="fa fa-times"></i>
                            </a>
                        </div>
                    ...
                    <div class="text-quote">
                        <div class="profile-quote">
                            @if(isset($value['aircraft_owner']['profile_image']) && $value['aircraft_owner']['profile_image']!=null && file_exists($operator_profile_base_img_path.$value['aircraft_owner']['profile_image']))
                            <img src="{{get_resized_image($value['aircraft_owner']['profile_image'] ,$operator_profile_base_img_path,40,40)}}">
                            @else
                            <img src="{{url('/')}}/uploads/default/no-img-user-profile-old.jpeg">
                            @endif
                        </div>
                        <div class="profile-text-quote">{{$value['aircraft_owner']['first_name'] or ''}} {{$value['aircraft_owner']['last_name'] or ''}}</div>
                    </div>
                ...
                <div class="content-date-quote">
                    <span class="june">
                        ...
                        {{get_formated_date($value['from_date']) }}
                    </span> 
                    <span class="june to-date-section">
                        <span class="from">
                            <i class="far fa-calendar-alt"></i> To
                        </span> 
                        {{get_formated_date($value['to_date']) }}
                    </span> 
                </div>
                <div class="price-quote">
                    <span class="price"><i class="fa fa-dollar-sign"></i> Price</span> {{isset($value['aircraft']['price_per_hour'])? get_formatted_price($value['aircraft']['price_per_hour'],$cny_price) : '' }}
                </div>
               <!--  <div class="button-quote request-button">
                    @if($value['status'] == 'REQUESTED')
                        <div class="accept" style="width: 100%">
                            <a href="javascript:void(0)">{{ trans('general.requested') }}</a>
                        </div>
                    @elseif($value['status'] == 'REPLIED')
                        <div class="accept"><a href=" {{url('/')}}/user/accept_quotation/{{base64_encode($value['rfq_id'])}}">{{ trans('general.accept') }}</a></div>
                        <div class="accept reject"><a href="{{url('/')}}/user/reject_quotation/{{base64_encode($value['rfq_id'])}}">{{ trans('general.cancel') }}</a></div>
                    @elseif($value['status'] == 'ACCEPTED')
                        {{-- @if($value['is_contract_generated'] == '0')
                            <div class="accept"><a href="javascript:void(0)">{{ trans('general.accepted') }}</a></div>
                        @else --}}
                            <div class="accept" style="width: 100%"><a href="javascript:void(0)">{{ trans('general.accepted') }}</a></div>  
                        {{-- @endif --}}
                    @elseif($value['status'] == 'REJECTED')
                    <div class="accept" style="width: 100%"><a href="javascript:void(0)">{{ trans('general.rejected') }}</a></div>
                    @endif

                    {{-- @if($value['is_contract_generated'] == '1')
                    @elseif( $value['is_contract_generated'] != '1' && $value['status'] !='REJECTED')
                        <div class="accept reject"><a href="{{url('/')}}/user/reject_quotation/{{base64_encode($value['rfq_id'])}}">{{ trans('general.cancel') }}</a></div>
                    @endif --}}
    ...
    <script type="text/javascript">
        ...
        $('#search').on("keypress", function(e) {
            if (e.keyCode == 13) {
                var search = $(this).val();
                window.location.href = "{{ url('/') }}/user/requested_quotations?search="+search
            }
        });
    ...
    $('.reset-form').click(function(){
        $("#search").val('');
        window.location.href = "{{ url('/') }}/user/requested_quotations";
    });
