...
<section class="gray-bg-main-section">
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            if(isset($value['get_owner_details']['profile_image']) && $value['get_owner_details']['profile_image']!=null && file_exists($operator_profile_base_img_path.$value['get_owner_details']['profile_image']))
            {
                $profile_image = get_resized_image($value['get_owner_details']['profile_image'] ,$operator_profile_base_img_path,40,40);

                $profile_img_pop = get_resized_image($value['get_owner_details']['profile_image'] ,$operator_profile_base_img_path,40,40);
            }else{
                $profile_img_pop = $profile_image =  url('/')."/uploads/default/no-img-user-profile-old.jpeg";
            }
            ...
            <div class="col-sm-4 col-md-4 col-lg-4">
                ...
                <a href="{{ url('/') }}/details/{{ base64_encode($value['get_aircraft_details']['id']) }}">
                    <div class="img-quote">
                        ...
                        <div class="text-quote">
                            ...
                            <div class="profile-text-quote">{{$value['get_owner_details']['first_name'] or ''}} {{$value['get_owner_details']['last_name'] or ''}}</div>
                        </div>
                ...
                <?php $reviews =    get_aircraft_reviews($value['reservation_id'],$value['user_id']) ;
                    ?>
                    @if($reviews)
                    ...
                    @else
                        <div class="accept"><a href="javascript:void(0)" id="review_modal" class="review_modal openModel"
                            ...
                            data-name="{{ $value['get_owner_details']['first_name'] or 'N/A'  }}"
                            data-last-name="{{ $value['get_owner_details']['last_name'] or 'N/A'  }}"
                    ...
                    @endif
                            ...
                            <div class="accept reject"><a href="{{ url('/').'/user/view_contract/'.base64_encode($value["id"]) }}">{{ trans('general.view_contract') }}</a></div>
                        </div>   
                    </div>
                </div>
                @endforeach
...
<script type="text/javascript">
    ...
    $('#search').on("keypress", function(e) {
        if (e.keyCode == 13) {
            var search = $(this).val();
            window.location.href = "{{ url('/') }}/user/completed_bookings?search="+search
        }
    });
    ...
    $('.openModel').on('click',function(){
        ...
        var url             = "{{url('/')}}/user/reviews/"+btoa(id)+'/'+btoa(aircraft_id);
    ...
    $('.reset-form').click(function(){
        $("#search").val('');
        window.location.href = "{{ url('/') }}/user/completed_bookings";
    });
    </script>
    @endsection