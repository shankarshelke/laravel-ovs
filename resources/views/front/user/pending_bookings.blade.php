            ...
            <div class="row">
                ...
                <div class="col-sm-4 col-md-4 col-lg-4">
                    ...
                    <div class="button-quote request-button main-button1">
                        <div class="accept">
                            <a href="{{ url('/').'/user/view_contract/'.base64_encode($value["id"]) }}">{{ trans('general.view_contract') }}</a>
                        </div>
                        @if($value['status'] != 'CANCELLED' && $value['cancellation_status'] == '' && $day_remain > 0)
                            <div class="accept reject">
                                <a href="{{ url('/').'/user/cancel_book_req/'.base64_encode($value['id']) }}" onclick="return confirm_action(this,event,'Do you really want to cancel this booking ?')">{{ trans('general.cancel') }} {{ trans('general.request') }}</a>
                            </div>
                        @elseif($value['status'] != 'CANCELLED' && $value['cancellation_status'] == 'REQUESTED' && $day_remain > 0)
                            <div class="accept reject">
...
<script type="text/javascript">
  ...
  $('.reset-form').click(function(){
        $("#search").val('');
        window.location.href = "{{ url('/') }}/user/pending_bookings";
    });
</script>
